...
int main(int argc, char** argv) {
    ...
    while (plyrScr[scrTrck]<MAX_SCR&&cmpScr[scrTrck]<MAX_SCR) {
        ...
        numGen(randNum, cardNum);
            ...
            //Card 2
            //Assign Value Utilizing character arrays and String Addition
            number=cardFce[randNum[1]%13];
            suit=cardSt[randNum[1]/13];
            //Add Strings to Create Card Value
            card2=number+suit;
            //card2+=cardFce[randNum[1]%13]+cardSt[randNum[1]/13];
            

            //Card 3
            //Assign Value Utilizing 2D Array
            number=cards[0][randNum[2]%13];
            suit=cards[randNum[2]/13][0];
            card3=number+suit;       
            /*//Card 4
            ...
            for (int card=1;card<=cardNum;card++) {
                ...
                in>>cardIn;
                if (randNum[5]==card)  card6=cardIn;   //Last Card in Player's Hand
                if (randNum[6]==card)  cStart=cardIn;  //Starter Card for the Round
                if (randNum[7]==card)  cmpCrd1=cardIn; //First Card for Computer
                if (randNum[8]==card)  cmpCrd2=cardIn; //Second Card for Computer
                if (randNum[9]==card) cmpCrd3=cardIn; //Third Card for Computer
                if (randNum[10]==card) cmpCrd4=cardIn; //Fourth Card for Computer
                if (randNum[11]==card) crib1=cardIn;   //First Card for the Computer Crib
                if (randNum[12]==card) crib2=cardIn;   //Second Card for the Computer Crib
        }
